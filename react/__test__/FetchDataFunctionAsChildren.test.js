...
describe('FetchDataFunctionAsChildren', () => {
	...
	it('fetches data from url', (done) => {
		...
		component = shallow(<FetchDataFunctionAsChildren url="www.dummy.com">{() => {}}</FetchDataFunctionAsChildren>)
	...
	it('passes data to its children', (done) => {
		...
		component = shallow(
			<FetchDataFunctionAsChildren>{(data) => <DummyComponent displayData={data} />}</FetchDataFunctionAsChildren>
		)
